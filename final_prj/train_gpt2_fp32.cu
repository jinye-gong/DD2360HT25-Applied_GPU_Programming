...
// Integer ceil division, handy for CUDA grid sizing.
#define CEIL_DIV(M, N) (((M) + (N)-1) / (N))
// CUDA error checking
// CUDA error-check helper (prints message and exits on failure).
...
// cuBLAS error-check helper (prints status code and exits on failure).
...
// Encoder forward: out = token_embedding[inp] + position_embedding[t] (vectorized as float4).
...
// LayerNorm forward: warp-reductions for mean/variance, then affine transform (weight+bias).
...
// Tiled matmul forward (custom GEMM). Each block computes a large output tile; each thread accumulates a small sub-tile.
...
// Launcher: embedding + positional encoding.
...
// Launcher: Linear layer forward (GEMM) (+ optional bias).
...
// Launcher: multi-head self-attention forward.
...
// Launcher: Linear layer backward (dX, dW, dBias).
...
// GPT-2 hyperparameters / model config.
typedef struct {
...
// Parameter tensors (all float32), laid out contiguously in params_memory.
typedef struct {
    float* wte; // (V, C)
    float* wpe; // (maxT, C)
    float* ln1w; // (L, C)
...
// Forward activations (allocated lazily on first forward pass).
typedef struct {
    float* encoded; // (B, T, C)
    float* ln1; // (L, B, T, C)
...
// Backward-pass scratch activations (reused per layer to save memory).
typedef struct {
    float* bt4c; // (B, T, 4*C)
...
// Full model state: params, grads, optimizer state, activations, and runtime buffers.
typedef struct {
    GPT2Config config;
...
// Load a GPT-2 checkpoint (header + parameters) and upload weights to the GPU.
...
// Full GPT-2 forward pass. If targets != NULL, also computes the cross-entropy loss.
...
// Full backward pass. Reuses buffers aggressively to reduce activation memory.
...
// AdamW optimizer step on GPU (updates params in-place).
...
// Free all GPU/CPU buffers owned by the model.
...
// Enable TF32 tensor-op math on Ampere+ (SM80+) for faster FP32 GEMMs (reduced mantissa).
    ...
    for (int step = 0; step <= train_num_batches; step++) {
        ...
        // do a training step
// --- Timing: measure one training iteration (we synchronize to get accurate GPU timings).
...
// <<< 在这里保存 >>>
gpt2_save_checkpoint(&model, "weight/gpt2_trained.bin");
