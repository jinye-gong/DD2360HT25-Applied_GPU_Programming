...
__global__ void add_bias_kernel(float* out, const float* bias, int BT, int OC) {
    ...
    int n = BT * OC;
    if (idx < n) {
        int oc = idx % OC;   // out 是 row-major: [bt][oc]
        out[idx] += bias[oc];
    }
}


// Linear layer matmul wrapper: out = inp * weight^T (plus optional bias).
...
// cuBLAS SGEMM; with TF32 math mode, cuBLAS may dispatch Tensor Core kernels.
    cublasCheck(cublasSgemm(
        ...
        OC, BT, C,
        &one,
        weight, C,   // lda = C (因为把 weight 当成 (C,OC) 的 column-major)
        inp, C,      // ldb = C (把 inp 当成 (C,BT) 的 column-major)
        &zero,
        out, OC      // ldc = OC (out 当成 (OC,BT) 的 column-major)
    ));
...
// Full GPT-2 forward pass (encoder + L transformer blocks + final head + optional loss).
...
// Full GPT-2 backward pass (reverse of forward; reuses buffers to save memory).
...
// Select math mode: allow TF32 tensor-op math on Ampere+ when enabled.
	...
	// <<< 在这里保存 >>>
	gpt2_save_checkpoint(&model, "weight/gpt2_update1_trained.bin");
