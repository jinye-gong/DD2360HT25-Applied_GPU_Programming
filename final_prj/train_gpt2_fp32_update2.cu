...
change matmul_forward()  to tensor core
...
// Helper macro: integer ceil-division (used to size CUDA grids).
#define CEIL_DIV(M, N) (((M) + (N)-1) / (N))

// Simple elementwise cast: float -> half (used to feed FP16 GEMM).
__global__ void f32_to_f16(__half* dst, const float* src, int n) {
...
// CUDA error checking
// CUDA runtime error checking helper (abort on failure).
...
// cuBLAS status checking helper (abort on failure).
...
// Global cuBLAS handle + compute type (e.g., FP32 or TF32-fast).
...
// Default matmul wrapper for most linear layers (FP32/TF32 via cuBLAS SGEMM).
...
// Self-attention forward: QKV projection + batched GEMMs + softmax + output projection.
...
// cuBLAS SGEMM; with TF32 math mode, cuBLAS may dispatch Tensor Core kernels.
...
// FP16 shadow copy of MLP fc weights (kept in sync with FP32 weights during training).
    __half* fcw_half;       // half copy of params.fcw: size L*(4C*C)
    __half* fc_inp_half;    // half scratch for fc input: size (B*T*C)
...
// MLP first FC (C -> 4C) using FP16 inputs/weights + Tensor Core GEMMEx; output stays FP32.
void matmul_forward_mlp_fc_fp16(GPT2* model,
                                float* out,
                                const float* inp,
                                const float* bias,
                                int B, int T, int C, int layer_idx) {
    int BT = B * T;
    int OC = 4 * C;

    int n_inp = BT * C;
    int threads = 256;
    int blocks = (n_inp + threads - 1) / threads;
// Cast the current activation tile to FP16 scratch on GPU.
    f32_to_f16<<<blocks, threads>>>(model->fc_inp_half, inp, n_inp);
    cudaCheck(cudaGetLastError());

    const __half* w_h = model->fcw_half + (size_t)layer_idx * (size_t)OC * (size_t)C;

    const float alpha = 1.0f;
    const float beta  = 0.0f;

// cuBLAS GEMMEx with CUDA_R_16F inputs; CUBLAS_GEMM_DEFAULT_TENSOR_OP enables Tensor Core paths.
    cublasCheck(cublasGemmEx(
        ...
        &alpha,
        w_h,                CUDA_R_16F, C,
        model->fc_inp_half, CUDA_R_16F, C,
        &beta,
        out,                CUDA_R_32F, OC,
        CUBLAS_COMPUTE_32F,
    ...
    if (bias != NULL) {
        int n = BT * OC;
        int b2 = (n + threads - 1) / threads;
        add_bias_kernel<<<b2, threads>>>(out, bias, BT, OC);
        cudaCheck(cudaGetLastError());
    }
}
...
void gpt2_build_from_checkpoint(GPT2 *model, const char* checkpoint_path) {
    ...
    // --- create half copy for MLP fc weights (params.fcw) ---
    int L = model->config.num_layers;
    int C = model->config.channels;
    size_t fcw_elems = (size_t)L * (size_t)(4*C) * (size_t)C;

    cudaCheck(cudaMalloc((void**)&model->fcw_half, fcw_elems * sizeof(__half)));

    int threads = 256;
    int blocks = (int)((fcw_elems + threads - 1) / threads);
    f32_to_f16<<<blocks, threads>>>(model->fcw_half, model->params.fcw, (int)fcw_elems);
    cudaCheck(cudaGetLastError());
    ...
    model->fc_inp_half = NULL;

}

// Full GPT-2 forward pass (this variant uses FP16 GEMM for the MLP first FC).
void gpt2_forward(GPT2 *model, int* inputs, int* targets, int B, int T) {
    ...
    if(model->acts_memory == NULL) {
        ...
        cudaCheck(cudaMalloc((void**)&model->fc_inp_half, (size_t)B * T * C * sizeof(__half)));
...
// Keep FP16 shadow weights up-to-date: params.fcw changes every optimizer step.
    // --- sync fp16 copy of MLP fc weights every forward ---
    // fcw is laid out as [L][4C][C] in float
    size_t fcw_elems = (size_t)L * (size_t)(4 * C) * (size_t)C;

    int threads = 256;
    int blocks = (int)((fcw_elems + threads - 1) / threads);

    // 注意：f32_to_f16 的 n 参数是 int，这里 elems 不会超过 int（对 GPT-2 small 是 OK 的）
    f32_to_f16<<<blocks, threads>>>(model->fcw_half, params.fcw, (int)fcw_elems);
    cudaCheck(cudaGetLastError());
    ...
    for (int l = 0; l < L; l++) {
        ...
        matmul_forward_mlp_fc_fp16(model, l_fch, l_ln2, l_fcb, B, T, C, l);
...
// Full GPT-2 backward pass (still uses FP32 weights/gradients).
...
// AdamW parameter update kernel (runs on GPU).
...
void gpt2_free(GPT2 *model) {
    ...
    cudaCheck(cudaFree(model->fcw_half));
    cudaCheck(cudaFree(model->fc_inp_half));
...
// Save current GPU weights back to a .bin checkpoint (header + FP32 params).
void gpt2_save_checkpoint(GPT2 *model, const char* out_path) {
    mkdir("weight", 0755);

    const char* in_ckpt = "gpt2_124M.bin";
    FILE *fin = fopenCheck(in_ckpt, "rb");
    int header[256];
    freadCheck(header, sizeof(int), 256, fin);
    fcloseCheck(fin);

    size_t n = model->num_parameters;
    float* cpu_params = (float*)mallocCheck(n * sizeof(float));
    cudaCheck(cudaMemcpy(cpu_params, model->params_memory, n * sizeof(float), cudaMemcpyDeviceToHost));

    FILE *fout = fopenCheck(out_path, "wb");
    fwrite(header, sizeof(int), 256, fout);
    fwrite(cpu_params, sizeof(float), n, fout);
    fcloseCheck(fout);

    free(cpu_params);
    printf("[save] wrote checkpoint: %s (%zu floats)\n", out_path, n);
}
...
// ----------------------------------------------------------------------------
// main training loop
// Program entry point: parse CLI, init GPU/cuBLAS, train loop, print timing, save if needed.
...
// Create cuBLAS handle (one-time init).
    cublasCheck(cublasCreate(&cublas_handle));
    // TF32 precision is equivalent to torch.set_float32_matmul_precision('high')
    int enable_tf32 = deviceProp.major >= 8 ? 1 : 0;
    cublas_compute_type = enable_tf32 ? CUBLAS_COMPUTE_32F_FAST_TF32 : CUBLAS_COMPUTE_32F;
    cublasMath_t cublas_math_mode = enable_tf32 ? CUBLAS_TF32_TENSOR_OP_MATH : CUBLAS_DEFAULT_MATH;
// Select math mode: allow TF32 tensor-op math on Ampere+ when enabled.
    ...
    for (int step = 0; step <= train_num_batches; step++) {
        ...
        // 1) data: 从 DataLoader 取一个 batch（纯 CPU）
        clock_gettime(CLOCK_MONOTONIC, &t0);
        dataloader_next_batch(&train_loader);
        clock_gettime(CLOCK_MONOTONIC, &t1);
        double iter_data_s = (t1.tv_sec - t0.tv_sec) +
                             (t1.tv_nsec - t0.tv_nsec) / 1e9;
        total_data_s += iter_data_s;

        // 2) forward: 前向传播（包含 GPU 计算）
        clock_gettime(CLOCK_MONOTONIC, &t0);
        gpt2_forward(&model, train_loader.inputs, train_loader.targets, B, T);
        // 等待 forward 里所有 CUDA kernel 完成
        cudaCheck(cudaDeviceSynchronize());
        clock_gettime(CLOCK_MONOTONIC, &t1);
        double iter_forward_s = (t1.tv_sec - t0.tv_sec) +
                                (t1.tv_nsec - t0.tv_nsec) / 1e9;
        total_forward_s += iter_forward_s;

        // 3) zero_grad: 清梯度（一般很快，这里不单独计时）
        gpt2_zero_grad(&model);

        // 4) backward: 反向传播（包含 GPU 计算）
        clock_gettime(CLOCK_MONOTONIC, &t0);
        gpt2_backward(&model);
        cudaCheck(cudaDeviceSynchronize());
        clock_gettime(CLOCK_MONOTONIC, &t1);
        double iter_backward_s = (t1.tv_sec - t0.tv_sec) +
                                 (t1.tv_nsec - t0.tv_nsec) / 1e9;
        total_backward_s += iter_backward_s;

        // 5) update: AdamW 参数更新（GPU 上的 adamw_kernel2）
        clock_gettime(CLOCK_MONOTONIC, &t0);
        gpt2_update(&model, learning_rate, 0.9f, 0.999f, 1e-8f, 0.0f, step+1);
        cudaCheck(cudaDeviceSynchronize());
        clock_gettime(CLOCK_MONOTONIC, &t1);
        double iter_update_s = (t1.tv_sec - t0.tv_sec) +
                               (t1.tv_nsec - t0.tv_nsec) / 1e9;
        total_update_s += iter_update_s;

        // 6) 额外的 sync：理论上这时应该已经几乎没工作可等了
        clock_gettime(CLOCK_MONOTONIC, &t0);
        cudaCheck(cudaDeviceSynchronize());
        clock_gettime(CLOCK_MONOTONIC, &t1);
        double iter_sync_s = (t1.tv_sec - t0.tv_sec) +
                             (t1.tv_nsec - t0.tv_nsec) / 1e9;
        total_sync_s += iter_sync_s;

        // 7) 整个 iteration 总时间（包含上面所有阶段）
        clock_gettime(CLOCK_MONOTONIC, &end);
        double time_elapsed_s = (end.tv_sec - start.tv_sec) +
                                (end.tv_nsec - start.tv_nsec) / 1e9;
        total_sum_iteration_time_s += time_elapsed_s;

        int tokens_per_second = (int)((double)(B * T) / time_elapsed_s);

        // 每步打印详细分解（单位：ms）
        printf(
            "step %4d/%d: train loss %f (%.3f ms, %d tok/s | data %.3f, fwd %.3f, "
            "bwd %.3f, upd %.3f, sync %.3f ms)\n",
            step + 1, train_num_batches, model.mean_loss,
            time_elapsed_s * 1000.0, tokens_per_second,
            iter_data_s * 1000.0, iter_forward_s * 1000.0,
            iter_backward_s * 1000.0, iter_update_s * 1000.0,
            iter_sync_s * 1000.0);
    ...
    // 额外打印各阶段的平均耗时和占比
    double steps = (double)train_num_batches;
    printf("\n=== Timing summary (per iteration average) ===\n");
    printf("data:    %.3f ms (%.1f %%)\n",
           total_data_s / steps * 1000.0,
           total_data_s / total_sum_iteration_time_s * 100.0);
    printf("forward: %.3f ms (%.1f %%)\n",
           total_forward_s / steps * 1000.0,
           total_forward_s / total_sum_iteration_time_s * 100.0);
    printf("backward:%.3f ms (%.1f %%)\n",
           total_backward_s / steps * 1000.0,
           total_backward_s / total_sum_iteration_time_s * 100.0);
    printf("update:  %.3f ms (%.1f %%)\n",
           total_update_s / steps * 1000.0,
           total_update_s / total_sum_iteration_time_s * 100.0);
    printf("sync:    %.3f ms (%.1f %%)\n",
           total_sync_s / steps * 1000.0,
           total_sync_s / total_sum_iteration_time_s * 100.0);
    printf("total:   %.3f ms (100.0 %%)\n",
           total_sum_iteration_time_s / steps * 1000.0);


// <<< 在这里保存 >>>
gpt2_save_checkpoint(&model, "weight/gpt2_update2_trained.bin");
